...
def get_decision_disruptions(disruptions: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
def find_shipment(shipment_id: str, shipments: list[dict[str, Any]]) -> dict[str, Any]:
...
def get_shipments() -> dict[str, Any]:
    ...
    for shipment in shipments:
        ...
        enriched["coldChainAlerts"] = [
            alert for alert in cold_chain_alerts if alert["shipmentId"] == shipment.get("id")
        ]
        enriched["assignedTruck"] = next(
            (truck for truck in fleet if truck.get("id") == shipment.get("assignedTruckId")), None
        )
...
def trigger_disruption(request: DisruptionRequest) -> dict[str, Any]:
    ...
    disruption = next(
        (item for item in disruptions if request.disruptionId and item.get("id") == request.disruptionId),
        None,
    )
    if disruption is None:
        affected_ids = request.affectedShipmentIds or [
            shipment["id"] for shipment in shipments if shipment.get("destination", "").endswith("Mumbai")
        ]
        disruption = {
            ...
            "impact": {"estimatedDelayHours": request.delayHours, "affectedShipmentIds": affected_ids},
        }
    else:
        disruption = deepcopy(disruption)
        disruption["status"] = request.status
    for shipment in shipments:
        if shipment.get("id") in (disruption.get("impact") or {}).get("affectedShipmentIds", []):
...
def optimize(request: OptimizeRequest) -> dict[str, Any]:
    shipments, fleet, disruptions = load_data()
    decision_disruptions = get_decision_disruptions(disruptions)
    shipment = find_shipment(request.shipmentId, shipments) if request.shipmentId else shipments[0]
    ...
    return {
        ...
        "mcpSignals": {"activeDisruptions": active_disruptions, "telemetry": telemetry},
        "blastRadius": calculate_blast_radius(shipment, active_disruption, fleet),
        "tradeoffMatrix": generate_tradeoff_matrix(shipment, decision_disruptions),
    }
...
def what_if(request: WhatIfRequest) -> dict[str, Any]:
    ...
    shipment = find_shipment(request.shipmentId, simulated_shipments) if request.shipmentId else simulated_shipments[0]
    ...
    return {
        ...
        "blastRadius": calculate_blast_radius(shipment, simulated_disruption, fleet),
        "tradeoffMatrix": generate_tradeoff_matrix(shipment, simulated_disruptions),
    }