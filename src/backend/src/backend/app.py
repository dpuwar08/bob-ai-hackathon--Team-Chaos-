...
# Mangum adapter for Netlify / AWS Lambda serverless execution
try:
...
app = FastAPI(
    title="BOB Logistics Optimizer",
    description="AI Logistics Control Tower Backend",
    version="1.0.0",
)
...
# This allows your frontend hosted on another domain
...
def get_decision_disruptions(
    ...
    live_mumbai = MCP_CONNECTOR.get_active_disruptions(
        "Mumbai"
    )["disruptions"]

    live_ids = {
        item.get("id")
        for item in live_mumbai
    }
...
def root() -> dict[str, Any]:
    return {
        ...
        "service": "Team Chaos - AI Logistics Control Tower",
        "version": "1.0.0",
        "docs": "/docs",
    }
...
def health() -> dict[str, str]:
    ...
    Useful for Render/Netlify/cloud monitoring.
    """
...
def get_shipments() -> dict[str, Any]:
    ...
    decision_disruptions = get_decision_disruptions(
        disruptions
    )

    cold_chain_alerts = flag_cold_chain_alerts(
        shipments
    )

    active_disruptions = MCP_CONNECTOR.get_active_disruptions(
        "Mumbai"
    )
    ...
    for shipment in shipments:
        ...
        enriched["riskScore"] = calculate_risk_score(
            shipment,
            decision_disruptions
        )
...
def trigger_disruption(
    ...
    disruption = next(
        (
            item
            for item in disruptions
            if request.disruptionId
            and item.get("id") == request.disruptionId
        ),
        None,
    )
    ...
    for shipment in shipments:
        ...
        if shipment.get("id") in (
            disruption.get("impact") or {}
        ).get(
            "affectedShipmentIds",
            []
        ):
...
def optimize(
    ...
    telemetry = MCP_CONNECTOR.get_telemetry_stream(
        shipment["id"]
    )
    ...
    active_disruption = next(
        (
            item
            for item in decision_disruptions
            if item.get("status") == "active"
        ),
        {
            "name": "No active disruption",
            "impact": {},
        },
    )
    ...
    return {
        ...
        "routes": evaluate_alternate_routes(
            shipment,
            decision_disruptions
        ),

        "idleTruckMatches": match_idle_trucks(
            shipment,
            fleet
        ),

        "coldChainAlerts": flag_cold_chain_alerts(
            [shipment]
        ),
